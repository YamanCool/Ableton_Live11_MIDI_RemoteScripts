...
class SoftwareController(MackieControlComponent):
    ...
    """ Master switch """
    def handle_master_switch_ids(self, switch_id, value):
        if switch_id == SID_MASTER:
            self.main_script().set_master_is_pressed(value == BUTTON_PRESSED)
    ...
    def handle_software_controls_switch_ids(self, switch_id, value):
        if switch_id == SID_MOD_SHIFT:
            ...
            if value == BUTTON_PRESSED:
                self.__toggle_detail_sub_view()
                """ Toggle Metronome """
        elif switch_id == SID_FUNC_METRONOME:
            if value == BUTTON_PRESSED:
                self.__toggle_metronome()
        elif switch_id == SID_FUNC_FOLLOW_SONG:
            if value == BUTTON_PRESSED:
                self.__toggle_follow_song()

    def refresh_state(self):
        """ Master switch """
        self.main_script().set_master_is_pressed(False)
