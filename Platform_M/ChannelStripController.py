...
class ChannelStripController(MackieControlComponent):
    ...
    def determine_assignment_mode(self, value):
        """
            Called from the main script
            Determine status which assignment mode is currently active
        """
        if not self.__main_display_controller.time_mode_status():
            if value == BUTTON_PRESSED:
                if self.__assignment_mode == CSM_IO:
                    status1 = u'Inputs/Outputs'
                elif self.__assignment_mode == CSM_SENDS:
                    status1 = u'Sends'
                elif self.__assignment_mode == CSM_VOLPAN:
                    status1 = u'Volume/Pan'
                elif self.__assignment_mode == CSM_PLUGINS:
                    status1 = u'Plug-ins'
                if self.__view_returns:
                    status1 = u'Return Tracks | ' + status1
                status1 += u' | ' + self.__master_strip.master_volume_status()
                status2 = self.__page_switch.rjust(36) + u''.ljust(20)
                duration = 30
                self.__main_display_controller.show_assignment_status(status1, status2, duration)

    def toggle_master_cue_volume(self, value):
        """ Called from the main script """
        if not self.__main_display_controller.time_mode_status():
            if value == BUTTON_PRESSED:
                self.__master_strip.toggle_master_volume()
                status1 = self.__master_strip.master_volume_status()
                status2 = u''.center(56)
                duration = 15
                self.__main_display_controller.show_assignment_status(status1, status2, duration)
    ...
    def __controlled_num_of_tracks(self):
        """ Return the number of tracks, depending on if we are in send_track mode or normal track mode """
        if self.__view_returns:
    ...
    def __set_assignment_mode(self, mode):
        ...
        self.__reassign_channel_strip_parameters(for_display_only=False)
        self.__update_channel_strip_strings()
        self.__update_page_switch_status()
        #if mode == CSM_PLUGINS:
    ...
    def __switch_to_prev_page(self):
        """
            Switch to the previous page in the non track strip modes (choosing plugins, or
            controlling devices)
        """
    ...
    def __switch_to_next_page(self):
        """
            Switch to the next page in the non track strip modes (choosing plugins, or
            controlling devices)
        """
    ...
    def __can_flip(self):
        """ Flip only in assignment mode CSM_VOLPAN and for Plug-ins parameter """
    ...
    def __update_page_switch_status(self):
        """ Visualize if the "prev" and "next" buttons can be pressed """
        if self.__can_switch_to_prev_page():
            self.__page_switch = u'<'
        else:
            self.__page_switch = u' '
    ...
    def __on_any_tracks_output_type_changed(self):
        """ Called as soon as any device chain has changed (devices where added/removed/swapped...) """
        self.__reassign_channel_strip_parameters(for_display_only=False)
        self.request_rebuild_midi_map()
