...
class Transport(MackieControlComponent):
    ...
    u"""Scrub Button wird nicht abgefragt, da er für Follow Arrangement verwendet wird"""
    ...
    def handle_jog_wheel_switch_ids(self, switch_id, value):
        if switch_id == SID_JOG_CURSOR_UP:
            ...
            if value == BUTTON_PRESSED:
                ...
                self.__cursor_repeat_delay = 0
                u"""Cursor Left Button LED ein"""
                self.send_midi((NOTE_ON_STATUS, SID_JOG_CURSOR_LEFT, BUTTON_STATE_ON))
                self.__on_cursor_left_pressed()
            elif value == BUTTON_RELEASED:
                self.__cursor_left_is_down = False
                u"""Cursor Left Button LED aus"""
                self.send_midi((NOTE_ON_STATUS, SID_JOG_CURSOR_LEFT, BUTTON_STATE_OFF))
        elif switch_id == SID_JOG_CURSOR_RIGHT:
            if value == BUTTON_PRESSED:
                self.__cursor_right_is_down = True
                self.__cursor_repeat_delay = 0
                u"""Cursor Right Button LED ein"""
                self.send_midi((NOTE_ON_STATUS, SID_JOG_CURSOR_RIGHT, BUTTON_STATE_ON))
                self.__on_cursor_right_pressed()
            elif value == BUTTON_RELEASED:
                self.__cursor_right_is_down = False
                u"""Cursor Right Button LED aus"""
                self.send_midi((NOTE_ON_STATUS, SID_JOG_CURSOR_RIGHT, BUTTON_STATE_OFF))
